...
1. 중력작용하는 부분을 제대로 테스트하지 않고 넘어감
2. 기준 블록의 위치가 큰 기준인지 작은 기준인지 제대로 읽지 않음 

위 두가지 때문에 손으로 직접 그려가며 문제 발생 부분을 찾아내느라 오래걸렸다. 시험장이었으면 제한 시간안에 못풀었을듯..

앞으로 함수 하나 구현할 때마다 테스트를 확실히하고 넘어가자 (그냥 넘어가면 찾는데 너무 오래걸림)

# 개선해야할 부분

1. 중력작용 부분의 구현이 어려워서 쓸대없는 반복문이 들어가있음

# 구현과정에서 발생한 실수

- 비어있는 2차원 배열을 선언하는데 "[[0]*size]*size" 이런식으로 선언했다가 서로 연결되어버려 Rotate 함수 구현에 문제가 발생했었다. 
- [[0]*size for _ in range(size)]  , 올바른 선언
